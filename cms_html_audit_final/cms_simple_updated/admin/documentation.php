...
if (isset($_POST['upload_doc']) && isset($_FILES['doc_files'])) {
    ...
    for ($i = 0; $i < $fileCount; $i++) {
        ...
        if (!in_array($ext, $allowedExtensions)) {
            $errors[] = "$originalName was skipped - only PDF, DOCX, or ZIP files are allowed.";
            continue;
        }
  ...
  <div class="card mb-4">
    <div class="card-body">
      ...
      <form method="POST" enctype="multipart/form-data" class="d-flex gap-2 flex-wrap">
        <input type="file" name="doc_files[]" accept=".pdf,.docx,.zip" multiple class="form-control" style="max-width:360px;" required>
        <button type="submit" name="upload_doc" class="btn btn-accent">Upload</button>
      </form>
      <small class="text-muted d-block mt-2">
        Accepts PDF, DOCX, or ZIP - select multiple at once, or upload a single ZIP bundle. This section is visible to admins only.
      </small>
    </div>
  </div>
